...
export async function authenticate(req: Request, _res: Response, next: NextFunction) {
    ...
    try {
        ...
        // SUPER_ADMIN can override org context via header
...
// Root-only authorization — only isRoot SUPER_ADMIN accounts
export function authorizeRoot(req: Request, _res: Response, next: NextFunction) {
    if (!req.user) return next(new AppError(401, 'Not authenticated'));
    if (!req.user.isRoot) return next(new AppError(403, 'Root account required'));
    next();
}
