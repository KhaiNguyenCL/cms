...
export async function listAllOrganizations(): Promise<OrgListRow[]> {
    const rows = await query<{
        ...
        `SELECT
            ...
            (SELECT COUNT(*) FROM devices d WHERE d."organizationId" = o.id AND d."deletedAt" IS NULL)                AS total_devices,
            (SELECT COUNT(*) FROM devices d WHERE d."organizationId" = o.id AND d."deletedAt" IS NULL AND d.status = 'ONLINE') AS online_devices,
            (SELECT COUNT(*) FROM media m WHERE m."organizationId" = o.id AND m."deletedAt" IS NULL)     AS total_media,
            (SELECT COALESCE(SUM(m."fileSize"), 0) FROM media m WHERE m."organizationId" = o.id AND m."deletedAt" IS NULL) AS total_media_size,
            (SELECT COUNT(*) FROM playlists p WHERE p."organizationId" = o.id AND p."isAutoGenerated" = false) AS total_playlists,
            (SELECT COUNT(*) FROM schedules s WHERE s."organizationId" = o.id)                           AS total_schedules,
            (SELECT COUNT(*) FROM devices d WHERE d."organizationId" = o.id AND d."deletedAt" IS NULL AND d."isLicensed" = true) AS licensed_devices
